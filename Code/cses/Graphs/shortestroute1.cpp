...
void solve()
{
	...
	vector<ll> distance(n + 1, 1e18);//distance of city i from city 1
	...
	set<array<ll, 2>> s;
	distance[1] = 0;//distance of city 1 from city 1 is 0
	s.insert({0, 1});//distance, city
	while(s.size())
	{
		ll city = (*s.begin())[1];
		ll cur_distance = (*s.begin())[0];
		s.erase(s.begin());
		if(processed[city])// move on
			continue;
		processed[city] = true;
